...
Coverage family: `COV_EXTRACTION_GAP`, `COV_UNSOURCED_COLUMN`, `MEMO_OUTLINE_MISSING`.
Requirements family: `REQUIREMENT_SOURCE_MISSING`, `REQUIREMENT_UNASSESSED`, `REQUIREMENT_EXTERNAL`,
`REQUIREMENT_SERVED_BY_NOTHING`, `TABLE_ANSWERS_NO_REQUIREMENT`; on ingest
`REQUIREMENT_TABLE_MISSING`, `REQUIREMENT_SECTION_MISSING`, `REQUIREMENT_REF_MISSING`,
`REQUIREMENT_DISPOSITION_INVALID`, `REQUIREMENT_LINK_INVALID`.
Freshness (Addendum A.1): `DOCSET_MOVED`, `DOCSET_UNRECORDED`, `DOCSET_UNOBSERVED`,
`DOCSET_NO_PROJECT`, `HARVEY_UNAVAILABLE`, `DOCSET_TOO_LARGE_TO_ENUMERATE`,
`DOCSET_POLL_BUDGET_SPENT`. Readiness (A.4): `COLUMN_NOT_VERIFIED`, `COLUMN_STALE`,
